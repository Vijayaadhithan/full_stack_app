...
export async function ensureDefaultAdmin() {
  ...
  const primaryDb = bootstrapDeps.db.primary;
  {
    ...
    for (const perm of requiredPermissions) {
      const exists = await primaryDb
        .select()
        .from(adminPermissions)
        .where(eq(adminPermissions.action, perm.action))
        .limit(1);
      if (exists.length === 0) {
        await primaryDb.insert(adminPermissions).values(perm);
      }
    }
    ...
    let role = (
      await primaryDb.select().from(adminRoles).where(eq(adminRoles.name, roleName)).limit(1)
    )[0];
    if (!role) {
      [role] = await primaryDb
        .insert(adminRoles)
    ...
    const perms = await primaryDb.select().from(adminPermissions);
    for (const p of perms) {
      const link = await primaryDb
        .select()
        .from(adminRolePermissions)
        .where(and(eq(adminRolePermissions.roleId, role.id), eq(adminRolePermissions.permissionId, p.id)))
        .limit(1);
      if (link.length === 0) {
        await primaryDb.insert(adminRolePermissions).values({ roleId: role.id, permissionId: p.id });
      }
    }

    // 4) Ensure env admin exists and is assigned Super Admin + has password set
    const byEmail = await primaryDb
      .select()
      .from(adminUsers)
