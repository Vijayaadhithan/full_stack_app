...
const emailPreferenceParamsSchema = z.object({
...
router.get(
  ...
  async (_req, res) => {
    try {
      ...
      const preferences = emailNotificationTypes.map((type) => {
        const defaultAudience = emailNotificationPreferences[type];
        const effectiveAudience = effective[type];
        const overrideAudience = overrides[type] ?? {};

        const recipients = {} as Record<
          EmailRecipient,
          { default: boolean; effective: boolean; overridden: boolean }
        >;

        for (const recipient of emailRecipients) {
          const defaultValue = defaultAudience?.[recipient] ?? true;
          const effectiveValue = effectiveAudience?.[recipient] ?? defaultValue;
      ...
      logger.error({ err: error }, "Failed to load email preferences");
      res
        .status(500)
        .json({
...
router.put(
  "/email-preferences/:notificationType/:recipientType",
  isAdminAuthenticated,
  checkPermissions(["manage_settings"]),
  async (req, res) => {
    const parsedParams = emailPreferenceParamsSchema.safeParse(req.params);
    if (!parsedParams.success) {
      return res.status(400).json(formatValidationError(parsedParams.error));
    }

    const { notificationType, recipientType } = parsedParams.data;
    if (!emailNotificationTypes.includes(notificationType as EmailNotificationType)) {
      return res.status(400).json({ message: "Invalid notification type" });
    }
    if (!emailRecipients.includes(recipientType as EmailRecipient)) {
      return res.status(400).json({ message: "Invalid recipient type" });
    }

    const parsedBody = emailPreferenceUpdateSchema.safeParse(req.body);
    if (!parsedBody.success) {
      return res.status(400).json(formatValidationError(parsedBody.error));
    }

    const typedNotificationType = notificationType as EmailNotificationType;
    const typedRecipient = recipientType as EmailRecipient;
    ...
    try {
      ...
      if (req.session?.adminId) {
        await db.insert(adminAuditLogs).values({
          ...
          resource: `${typedNotificationType}:${typedRecipient}=${enabled}`,
        });
      }
      ...
      logger.error({ err: error }, "Failed to update email preference");
      res.status(500).json({
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      await resetEmailPreference(typedNotificationType, typedRecipient);

      if (req.session?.adminId) {
        await db.insert(adminAuditLogs).values({
          adminId: req.session.adminId,
          action: "reset_email_preference",
          resource: `${typedNotificationType}:${typedRecipient}`,
        });
      }

      const defaultValue =
        emailNotificationPreferences[typedNotificationType]?.[typedRecipient] ??
        true;
      ...
      logger.error({ err: error }, "Failed to reset email preference");
      res.status(500).json({
        message: "Failed to reset email preference",
        detail: error instanceof Error ? error.message : String(error),
      });
    }
  },
