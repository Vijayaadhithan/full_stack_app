...
export function startLowStockDigestJob(storage: IStorage): void {
  ...
  getJobQueue()
    ...
    .then((jobs) => {
      ...
      if (!exists) {
        void addRepeatableJob(JOB_TYPE, {}, schedule, {
          ...
          logger.error(
            { err },
            "[LowStockDigest] Failed to schedule repeatable job",
          );
        });
      } else {
    ...
    .catch((err) => {
      logger.error(
        { err },
        "[LowStockDigest] Failed to inspect existing repeatable jobs",
      );
    });
  ...
  void addJob(
    ...
    logger.error(
      { err },
      "[LowStockDigest] Failed to enqueue startup run; falling back to inline execution",
    );
    void processLowStockDigest(storage);
  });
}
