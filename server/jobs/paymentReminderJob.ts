...
export function startPaymentReminderJob(storage: IStorage) {
  ...
  const job = async () => {
    try {
      ...
      const disputeCutoff = new Date(now.getTime() - disputeDays * 24 * 60 * 60 * 1000);
      ...
      for (const b of awaiting) {
        ...
        if (updatedAt < disputeCutoff) {
          await storage.updateBooking(b.id, { status: 'disputed', disputeReason: 'Payment confirmation overdue.' });
        } else if (updatedAt < reminderCutoff) {
          const provider = await storage.getService(b.serviceId!).then(s => s ? storage.getUser(s.providerId!) : null);
