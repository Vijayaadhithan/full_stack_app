...
const ALLOWED_ROLES = new Set(["customer", "provider", "shop", "worker"]);

type MetricName = PerformanceMetric["name"];

type UserRole = string | null | undefined;

type MetricPayload = Omit<PerformanceMetric, "page" | "timestamp"> & {
...
export function useClientPerformanceMetrics(role: UserRole) {
  const initializedRef = useRef<UserRole>(null);
