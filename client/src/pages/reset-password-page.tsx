  ...
  const onSubmit = async (data: ResetPasswordData) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage(t('resetPassword.successMessage') + ' ' + t('resetPassword.redirectingToLogin'));
        setTimeout(() => {
          navigate('/auth');
        }, 3000);
      } else {
      ...
      <div className="flex items-center justify-center min-h-screen bg-gray-100">
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            ...
            <Button onClick={() => navigate('/auth')} className="mt-4 w-full">{t('resetPassword.backToLogin')}</Button>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <Card className="w-full max-w-md">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <CardContent className="space-y-4">
            ...
            {message && <p className={message.includes(t('resetPassword.successMessage')) ? 'text-green-500' : 'text-red-500'}>{message}</p>}
          </CardContent>
          <CardFooter>
